...
static int _send(candev_t *candev, const can_frame_t *frame)
{
    ...
    *tx_buffer_element_t1 |= ((frame->flags & CANFD_BRS) ? 1 : 0) << FDCAN_SRAM_TXBUFFER_T1_BRS_Pos;
    ...
    *tx_buffer_element_t1 |= dlc_to_fddlc(frame->len) << FDCAN_SRAM_TXBUFFER_T1_DLC_Pos;
    ...
    /* Bit rate switching and a payload data length greater than 8 are specific to FDCAN.
       Thus consider frame as a FDCAN one only if one of those two specifics is fulfilled */
