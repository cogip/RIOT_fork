...
static uint32_t* get_message_ram(FDCAN_GlobalTypeDef *can) {
    ...
    DEBUG("%s: FDCAN%u message RAM address is %p\n",
          ...
          + (((uint32_t)can - FDCAN1_BASE) >> 10) * 0x350
          + ((uint32_t)can > FDCAN1_BASE ? 4 : 0))
          );
...
static uint32_t* get_flesa_message_ram(FDCAN_GlobalTypeDef *can) {
    DEBUG("%s: FDCAN%u FLESA messsage RAM address is %p\n",
          __func__, get_channel_id(can),
...
static uint32_t* get_message_ram_rx_fifo_address(FDCAN_GlobalTypeDef *can, uint8_t message_ram_rx_fifo) {
...
static int filter_is_set(FDCAN_GlobalTypeDef *can, uint8_t filter_id)
{
    ...
    if (filter_id < FDCAN_STM32_NB_STD_FILTER) {
        ...
        if (filter_id < (can->RXGFC & FDCAN_RXGFC_LSS) >> FDCAN_RXGFC_LSS_Pos) {
            ...
            /* Filter List Standard start at RAM address + FLSSA (0x00) + standard filter offset (4 Bytes * filter_id) */
        ...
        if (filter_id < (can->RXGFC & FDCAN_RXGFC_LSE) >> FDCAN_RXGFC_LSE_Pos) {
            ...
            /* Filter List Extended start at RAM address + FLESA (0x70) + extended filter offset (8 Bytes * filter_id) */
            ...
            DEBUG("%s: FDCAN%u extended filter messsage RAM F0 address is %p\n",
                  __func__, get_channel_id(can),
                  fle_ram_address_f0
                  );
            DEBUG("%s: FDCAN%u extended filter messsage RAM F1 address is %p\n",
                  __func__, get_channel_id(can),
                  fle_ram_address_f1
                  );
...
static int set_standard_filter(FDCAN_GlobalTypeDef *can, uint32_t fr1, uint32_t fr2, uint8_t filter_id, uint8_t fifo)
{
    /* Check filter ID */
    if (filter_id >= FDCAN_STM32_NB_STD_FILTER) {
    ...
    /* Filter List Standard start at RAM address + FLSSA (0x00) + standard filter offset (4 Bytes * filter_id) */
    uint32_t *fls_ram_address = get_message_ram(can) + FDCAN_SRAM_FLS_FILTER_SIZE * filter_id;
    DEBUG("%s: FDCAN%u standard filter messsage RAM address is %p\n",
          __func__, get_channel_id(can),
          fls_ram_address
          );
...
static int set_extended_filter(FDCAN_GlobalTypeDef *can, uint32_t fr1, uint32_t fr2, uint8_t filter_id, uint8_t fifo)
{
    /* Check filter ID */
    ...
    /* Set filter/mask */
    if ((fr1 & ~CAN_EFF_MASK) || (fr2 & ~CAN_EFF_MASK)) {
        DEBUG("%s: FDCAN%u extended filter %u ID(%lx) or mask(%lx) are not valid\n",
            __func__, get_channel_id(can),
            filter_id,
            fr1,
            fr2);
    ...
    *fle_ram_address_f1 |= fr1 & CAN_EFF_MASK;
...
/* Select set function for standard or extended filter. Extended filters can filter standard messages */
static int set_filter(FDCAN_GlobalTypeDef *can, uint32_t fr1, uint32_t fr2, uint8_t filter_id, uint8_t fifo) {
    if (filter_id < FDCAN_STM32_NB_STD_FILTER) {
...
static int get_can_filter(FDCAN_GlobalTypeDef *can, uint8_t filter_id, uint32_t *filter, uint32_t *mask)
{
    if (filter_id < FDCAN_STM32_NB_STD_FILTER) {
        if (filter_id >= (can->RXGFC & FDCAN_RXGFC_LSS) >> FDCAN_RXGFC_LSS_Pos) {
            DEBUG("ERROR: invalid standard filter ID %u\n", filter_id);
...
static inline void set_bit_timing(can_t *dev)
{
    ...
    can->NBTP |= (dev->candev.bittiming.phase_seg1 + dev->candev.bittiming.prop_seg - 1) << FDCAN_NBTP_NTSEG1_Pos;
    /* BRP */
    ...
    can->DBTP |= (dev->candev.fd_data_bittiming.phase_seg1 + dev->candev.fd_data_bittiming.prop_seg - 1) << FDCAN_DBTP_DTSEG1_Pos;
    /* BRP */
    ...
    can->TDCR |= (dev->candev.fd_data_bittiming.brp * dev->candev.fd_data_bittiming.phase_seg2) << FDCAN_TDCR_TDCO_Pos;
...
static int _send(candev_t *candev, const can_frame_t *frame)
{
    ...
    uint32_t* tx_buffer_element_t0 = get_message_ram_tx_buffer_address(can) + (put_index * FDCAN_SRAM_TXBUFFER_SIZE);
    ...
    if (frame->len > 64) {
        DEBUG("%s: FDCAN%u DLC is upper 64 bytes\n",
              __func__, get_channel_id(can));
        return -EINVAL;
    }

    /* Determine if it a standard or extended identifier */
    uint32_t xtd_bit = ((frame->can_id & CAN_EFF_FLAG) == CAN_EFF_FLAG ? 1 : 0) << FDCAN_SRAM_TXBUFFER_T0_XTD_Pos;
    DEBUG("%s: FDCAN%u xtd_bit = %lx\n",
          __func__, get_channel_id(can), xtd_bit);

    /* Shift standard ID of 18 bits */
    uint32_t id = (xtd_bit ? (frame->can_id & CAN_EFF_MASK) : ((frame->can_id & CAN_SFF_MASK) << FDCAN_SRAM_TXBUFFER_T0_ID_Pos));
    DEBUG("%s: FDCAN%u id = %lx\n",
          __func__, get_channel_id(can), id);

    uint32_t rtr_bit = ((frame->can_id & CAN_RTR_FLAG) == CAN_RTR_FLAG ? 1 : 0) << FDCAN_SRAM_TXBUFFER_T0_RTR_Pos;
    DEBUG("%s: FDCAN%u rtr_bit = %lx\n",
          __func__, get_channel_id(can), rtr_bit);

    *tx_buffer_element_t0 =
        ( FDCAN_SRAM_TXBUFFER_T0_ESI_RECESSIVE      /* The Error State Indicator (ESI) depends of error passive flag */
    ...
    *tx_buffer_element_t1 |= candev->fd_bitrate_switch_on ?
        FDCAN_SRAM_TXBUFFER_T1_BRS_ON : FDCAN_SRAM_TXBUFFER_T1_BRS_OFF;
    ...
    *tx_buffer_element_t1 |= dlc_to_fddlc(frame->len) << FDCAN_SRAM_TXBUFFER_T1_DLC_Pos;
    ...
    /* Bit rate switching and a payload data length greater than 8 are specific to FDCAN.
       Thus consider frame as a FDCAN one only if one of those two specifities is fullfilled */
    ...
    *tx_buffer_element_t1 |= ((candev->fd_bitrate_switch_on)
    ...
    for (uint8_t byte = 0; byte < frame->len && !rtr_bit; byte++) {
        //*tx_buffer_element_data |= (uint32_t)frame->data[byte] << ((3 - (byte % 4)) * 8);
...
static int read_frame(can_t *dev, can_frame_t *frame, int message_ram_rx_fifo)
{
    ...
    if (*rx_fifo_element_r0 & FDCAN_SRAM_RXFIFO_R0_XTD) {
        ...
        frame->can_id = (*rx_fifo_element_r0 & FDCAN_SRAM_RXFIFO_R0_ID) >> FDCAN_SRAM_RXFIFO_R0_ID_Pos;
    }
    ...
    frame->len = fddlc_to_dlc((*rx_fifo_element_r1 & FDCAN_SRAM_RXFIFO_R1_DLC) >> FDCAN_SRAM_RXFIFO_R1_DLC_Pos);
    ...
    switch (opt) {
        ...
        case CANOPT_FD_BITTIMING:
            ...
            if (value_len < sizeof(dev->candev.fd_data_bittiming)) {
                ...
                memcpy(&dev->candev.fd_data_bittiming, value, sizeof(dev->candev.fd_data_bittiming));
                mode = get_mode(can);
                if (mode == MODE_SLEEP) {
                    res = _wake_up(dev);
                    if (res != 0) {
    ...
    switch (opt) {
        ...
        case CANOPT_FD_BITTIMING:
            if (max_len < sizeof(dev->candev.fd_data_bittiming)) {
                ...
                memcpy(value, &dev->candev.fd_data_bittiming, sizeof(dev->candev.fd_data_bittiming));
                res = sizeof(dev->candev.fd_data_bittiming);
            }
            break;
...
static int _set_filter(candev_t *candev, const struct can_filter *filter)
{
    ...
    DEBUG("%s: FDCAN%u dev=%p, filter=0x%" PRIx32 "\n", __func__, get_channel_id(can), (void *)candev, filter->can_id);

    uint8_t i;
    for (i = 0; i < FDCAN_STM32_NB_FILTER; i++) {
        if (!filter_is_set(can, i)
                && ((i >= FDCAN_STM32_NB_STD_FILTER)    /* First free filter slot is an extended filter */
                || !(filter->can_id & CAN_EFF_FLAG))) { /* or the filter is standard */
...
static void tx_irq_handler(can_t *dev)
{
    ...
    if (can->IR & FDCAN_IR_PEA) {
        //DEBUG("%s: FDCAN%u PSR.LEC = %lx\n", __func__, get_channel_id(can), dev->conf->can->PSR & FDCAN_PSR_LEC);
...
static void irq_handler(can_t *dev)
{
    ...
    DEBUG("%s: FDCAN%u Got interrupts, can->IR = %lx\n", __func__, get_channel_id(can), dev->conf->can->IR);
